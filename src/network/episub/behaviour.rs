  ...
  fn inject_connection_established(
    ...
    if matches!(endpoint, ConnectedPoint::Dialer { .. }) {
      ...
      if self.local_node.is_some() {
        // for each topic that has hasn't enough active nodes,
...
impl Episub {
  ...
  fn request_join_for_starving_topics(&mut self, peer: PeerId) {
    // for each topic with view that has not reached minimum size of active
    // nodes, send a join request to any dialer
    self
      .topics
      .iter_mut()
      .filter(|(t, v)| {
